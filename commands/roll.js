...
export async function rollTheDice(interaction){
    ...
    if (rollType === "coin") {
        ...
        return await msgCtrl.sendMsgToIntercation(interaction, `${outcome}`);
    }

    if (rollType === "d6") {
        const roll = Math.floor(Math.random() * 6) + 1;
        return await msgCtrl.sendMsgToIntercation(interaction, `🎲 6 oldalú kocka dobás eredménye: **${roll}**`);
    }
    ...
    if (rollType === "d20") {
        ...
        return await msgCtrl.sendMsgToIntercation(interaction, `🎲 20 oldalú kocka dobás eredménye: **${roll}**${flavorText}`);
    }
}
